...
void fetch_even_index(const vector<complex<long double>>& src,  vector<complex<long double>>& des)
{
...
void fetch_odd_index(const vector<complex<long double>>& src,  vector<complex<long double>>& des)
{
...
vector<complex<long double>> RECURSIVE_FFT(const vector<complex<long double>>& a)
{
	//获得当前表示的多项式的长度
	long long n = a.size();
	// tmp用于临时存放返回值
...
vector<complex<long double>> RECURSIVE_IFFT_part(const vector<complex<long double>>& y)
{
	long long n = y.size();
	vector<complex<long double>> tmp;
	if (n == 1)
...
vector<complex<long double>> RECURSIVE_IFFT(const vector<complex<long double>>& y)
{
...
string RULE(const vector<complex<long double>>& r)
{
	vector<long long> A;
...
void DataProcessing(const string& a, const string& b, vector<complex<long double>>& s, vector<complex<long double>>& t)
{
    long long max;
    if (a.size() < b.size())
        max = b.size();
